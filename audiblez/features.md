...
*   **Phase 3.3: Process Calibre HTMLZ Output**
    ...
    *   Created `extract_chapters_from_calibre_html` in [`audiblez/core.py`](audiblez/core.py):
        *   Parses the extracted HTML file (from the HTMLZ archive) generated by Calibre using `BeautifulSoup`.
        *   Identifies chapter breaks primarily using `<h1>` and `<h2>` tags as delimiters.
        *   Extracts text content for each chapter.
        *   Returns a list of `SimpleNamespace` objects, each representing a chapter, with attributes like `short_name`, `extracted_text`, `is_selected`, and `chapter_index` to be compatible with the UI.
*   **Phase 3.4: Integrate Calibre Workflow into UI** [DONE]
    *   Modified `on_open_with_calibre` in [`audiblez/ui.py`](audiblez/ui.py):
        *   Prompts the user to select an ebook file (supports various formats like EPUB, MOBI, AZW3, PDF, etc.).
        *   Calls `convert_ebook_with_calibre` to convert the selected ebook to HTMLZ in a temporary directory. This function will also handle unzipping the HTMLZ to get the HTML file.
        *   If conversion and unzipping are successful, calls `extract_chapters_from_calibre_html` to process the extracted HTML.
        *   Populates the "Chapters" tab and book details in the UI using the extracted chapter data. This reuses parts of the `open_epub` logic but is adapted for the `SimpleNamespace` chapter objects from HTML.
        *   Cleans up the temporary HTMLZ output and extracted files after processing.
