...
*   **Phase 3.2: Implement Calibre Conversion and Path Handling** [DONE]
    ...
    *   Created `convert_ebook_with_calibre` in [`audiblez/core.py`](audiblez/core.py):
        ...
        *   Executes `ebook-convert <input_file> <output_directory/output.html>` via `subprocess.run`.
...
*   **Phase 3.3: Process Calibre HTML Output** [DONE]
    *   Created `extract_chapters_from_calibre_html` in [`audiblez/core.py`](audiblez/core.py):
        *   Parses the HTML file generated by Calibre using `BeautifulSoup`.
...
*   **Phase 3.4: Integrate Calibre Workflow into UI** [DONE]
    *   Modified `on_open_with_calibre` in [`audiblez/ui.py`](audiblez/ui.py):
        ...
        *   Calls `convert_ebook_with_calibre` to convert the selected ebook to HTML in a temporary directory.
        *   If conversion is successful, calls `extract_chapters_from_calibre_html` to process the HTML.
        ...
        *   Cleans up the temporary HTML output directory after processing.
